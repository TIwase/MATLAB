...
path = 'C:\Users\TakuyaIwase\Documents\MATLAB\Git\MATLAB\Master_thesis\BA\';

for nfnc = 1:4
    M = csvread([path 'F' num2str(nfnc) '\F' num2str(nfnc) '_1_pbest.csv']);
    pop = M(:,1:2);
    x_lb = getLb(nfnc);
    x_ub = getUb(nfnc);
    ...
    if nfnc == 2
        ...
        xlim([x_lb(1) x_ub(1)]);
        ylim([x_lb(2) x_ub(2)]);
        title('generation: 10000');
        xlabel('x1');
        ylabel('x2');
        grid on;
        saveas(gcf,[path '\results\F' num2str(nfnc) '.png']);
        continue;
